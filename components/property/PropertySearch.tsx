...
export default function PropertySearch({ 
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      // Update URL with search params - only include defined values
      ...
      if (selectedTypes.length > 0) searchParams.set('types', selectedTypes[0]); // Only first type
  ...
  return (
    <div className={`w-full max-w-5xl mx-auto bg-white rounded-full shadow-lg p-6 ${className}`}>
      <div className="flex flex-col md:flex-row gap-4 mb-4">
        ...
        <Popover open={isFilterOpen} onOpenChange={setIsFilterOpen}>
          <PopoverTrigger asChild>
            <Button variant="outline" className="h-14 px-6 flex items-center gap-2">
              <SlidersHorizontal className="h-5 w-5" />
              <span className="hidden sm:inline">Filters</span>
            </Button>
          </PopoverTrigger>
          <PopoverContent className="w-[90vw] max-w-md p-4" align="end">
        ...
        <Button 
          className="h-14 px-6 bg-primary hover:bg-primary/90 text-white"
